...
class AddFoundItem:
    ...
    def create_form(self):
        ...
        self.date_found_entry = ttk.Entry(
            form_frame,
            font=('Helvetica', 12),
            width=30
        )
        self.date_found_entry.grid(row=3, column=1, padx=10, pady=10, sticky='w')
        self.date_found_entry.insert(0, datetime.now().strftime('%Y-%m-%d'))
        ...
        submit_btn = ttk.Button(
            form_frame,
            text="Submit Report",
            command=self.submit_found_item,
    ...
    def submit_found_item(self):
        ...
        date_found = self.date_found_entry.get().strip()
        ...
        if not date_found:
            messagebox.showerror("Error", "Please enter the date found")
            return
